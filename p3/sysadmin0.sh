...
### Constantes
TITLE="Información de los procesos."
...
if [ "${online}" -eq "${offline}" ]; then
	...
	for u in ${userlist}; do
		getmax_cputime_process "${u}"
		getmax_exetime_process "${u}"
		printf "%20s %5i %5i %20s %10s %20s %10s %12i %15i\n" "${u}" "$(id -u ${u})" "$(id -g ${u})" "${cpu_n}" "${cpu_t}" "${exe_n}" "${exe_t}" "$(lsof -u ${u} 2> /dev/null | sed "1d" | wc -l)" "$(ps -u ${u} -U ${u} | sed "1d" | wc -l)"
	done
	echo
...
if [ "${kill}" -eq "1" ]; then
	for pid in $(ps -A -o pid --sort=pid --no-headers); do
		if [ "$(lsof -p ${pid} | sed "1d" | wc -l)" -gt "${killparam}" ]; then
			kill ${pid} 2> /dev/null || echo "paso un error ahi"
		fi
	done
fi
